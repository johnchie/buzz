    ...
    <div class="same-padd music-card" id="events">
      <div class="wrap">
      	<div class="filter-row-box">
              <ul id="filters">
                <li><a href="javascript:void(0)" data-filter="*" class="filter current" title="All">All</a></li>
                <?php foreach($categorys as $category): ?>
                <li><a href="javascript:void(0)" data-filter=".<?php echo preg_replace('/[^a-z]/', '', strtolower($category->name)); ?>" class="filter" title="{{$category->name}}">{{$category->name}}</a></li>
                <?php endforeach; ?>
              </ul>
        </div><!--/.filter-row-box -->
        <div id="eventslist">
